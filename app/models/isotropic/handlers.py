...
from app.models.isotropic.shema import IsotropicResponse, IsotropicUploadRequest, IsotropicFitResponse, \
    IsotropicPredictResponse

router = APIRouter(prefix="/models/isotropic", tags=["isotropic"])
...
@router.post("/upload_model", response_model=IsotropicResponse,
             ...
             summary="Upload isotropic model file",
...
async def upload_model(
    ...
    if not body.file.filename.lower().endswith((".csv", ".xls", ".xlsx")):
        logger.warning("Unsupported file format: %s", body.file.filename)
    ...
    print(f"Загружено: {body.file.filename}, модель: {body.hyperlastic_model}, функция ошибки: {body.error_function}")
...
@router.post("/predict", response_model=IsotropicPredictResponse,
             summary="Predict using isotropic model",
