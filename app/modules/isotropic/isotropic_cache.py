...
class IsotropicCache:
    ...
    async def set_model_params(self, session_id: str, hyperelastic_model: str) -> None:
    ...
    async def get_model_params(self, session_id) -> str:
        key = f"{self._key_prefix(session_id)}:params"
        async with self.redis as redis:
            hyperelastic_model = await redis.hget(key, "hyperelastic_model")
        return hyperelastic_model.decode('utf-8')
    ...
    async def del_all(self, session_id: str) -> None:
        key_data = f"{self._key_prefix(session_id)}:isotropic_data"
        key_info = f"{self._key_prefix(session_id)}:params"
        async with self.redis as redis:
            await redis.delete(key_data)
            await redis.delete(key_info)
